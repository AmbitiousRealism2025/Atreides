...
describe('End-to-End CLI Tests', () => {
  ...
  describe('Init Command', () => {
    ...
    it('should create .claude directory in full mode', async () => {
    ...
    it('should NOT create .claude directory in minimal mode', async () => {
      ...
      // .claude directory should NOT exist in minimal mode
      const claudeDirExists = await fs.pathExists(join(testDir, '.claude'));
  ...
  describe('Doctor Command', () => {
    ...
    it('should detect invalid JSON in settings.json', async () => {
      // Write invalid JSON
      await fs.writeFile(join(testDir, '.claude', 'settings.json'), 'not valid json');
  ...
  describe('Full Workflow Integration', () => {
    ...
    it('should work with minimal mode workflow', async () => {
      ...
      expect(await fs.pathExists(join(testDir, '.claude'))).toBe(false);

      // Doctor should still work (project detected)
      const doctorResult = await runCli(['doctor'], { cwd: testDir });
      // In minimal mode without .claude, doctor might show warnings but shouldn't error
      expect(doctorResult.stdout).toContain('Health Check');
    });
  });
