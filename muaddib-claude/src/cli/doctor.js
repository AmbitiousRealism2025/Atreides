 ...
 * @param {Array} issues - Array to collect critical issues
 * @param {Array} warnings - Array to collect warnings
 * @returns {Promise<void>}
 */
async function checkGlobal(options, issues, warnings) {
  console.log();
  ...
  if (globalExists) {
    ...
    return; // Can't check more if global dir missing
  }
...
/**
 * Check project installation health
 * @param {object} options - Command options
 * @param {Array} issues - Array to collect critical issues
 * @param {Array} warnings - Array to collect warnings
 * @returns {Promise<boolean>} True if project was found
 */
async function checkProject(options, issues, warnings) {
...
/**
 * Attempt to auto-fix warnings that have autoFix functions
 * @param {Array} issues - Critical issues (shown as manual fixes)
 * @param {Array} warnings - Warnings with potential autoFix
 * @returns {Promise<void>}
 */
async function autoFix(issues, warnings) {
  console.log();
 ...
 * @returns {Promise<void>}
 */
async function displaySummary(options, issues, warnings) {
  console.log();
...
async function runDoctor(options) {
  ...
  // Run all checks
  await checkGlobal(options, issues, warnings);
  await checkProject(options, issues, warnings);

  // Display results
  await displaySummary(options, issues, warnings);
